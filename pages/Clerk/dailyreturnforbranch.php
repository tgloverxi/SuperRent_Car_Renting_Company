<html>
    ...
    <body>
        <div class="header">
            <h1>Daily Return Report for Specific Branch</h1>

            <form action = "dailyreturnforbranch.php" method="POST">
                <fieldset class="box">
...
//            echo "<br> here's the result: " . var_dump(oci_fetch_row($statement)) ."<br>";
        ...
        function printResult($result) { //prints results from a select statement
            ...
            echo "<h3>All the Cars that returned back today for the specific branch</h3>";
            $list = array();
            while ($r = OCI_Fetch_Array($result, OCI_BOTH)) {
                array_push($list, $r);
            }
            if (sizeof($list) != 0) {
                echo "<tr><th>Vehicle License</th><th>Make</th><th>Model</th><th>Year</th><th>Color</th><th>Odometer</th><th>Vehicle Type</th><th>City</th><th>Location</th></tr>";
                foreach ($list AS $row) {
                    echo "<tr><td>" . $row["0"] . "</td><td>" . $row["1"] . "</td><td>" . $row["2"] . "</td><td>" . $row["3"] . "</td><td>" . $row["4"] . "</td><td>" . $row["5"] . "</td><td>" . $row["6"] . "</td><td>" . $row["8"] . "</td><td>" . $row["7"] . "</td></tr>"; //or just use "echo $row[0]"
                }
            } else {
        ...
        function printResultPerCat($result) { //prints results from a select statement
            echo "<center><table></center>";
            echo "<h3>The number of vehicles returned back per category for the specific branch</h3>";
            $list = array();
            while ($r = OCI_Fetch_Array($result, OCI_BOTH)) {
                array_push($list, $r);
            }
        ...
        function printResultPerBranch($result) { //prints results from a select statement
            echo "<center><table></center>";
            echo "<h3>The number of vehicles returned back for the specific branch</h3>";
            $list = array();
            while ($r = OCI_Fetch_Array($result, OCI_BOTH)) {
                array_push($list, $r);
            }
        ...
        function printTotalRevenueForBranch($result) { //prints results from a select statement
            echo "<center><table></center>";
            echo "<h3>The total revenue for specific branch</h3>";
            $list = array();
            while ($r = OCI_Fetch_Array($result, OCI_BOTH)) {
                array_push($list, $r);
            }
        ...
        function printTotalRevenueForCat($result) { //prints results from a select statement
            echo "<center><table></center>";
            echo "<h3>The total revenue per category for specific branch</h3>";
            $list = array();
            while ($r = OCI_Fetch_Array($result, OCI_BOTH)) {
                array_push($list, $r);
            }
            if (sizeof($list) != 0) {
                echo "<tr><th>Revenue</th><th>City</th><th>Location</th><th>Vehicle Type</th></tr>";
                foreach ($list AS $row) {
                    echo "<tr><td>" . $row["0"] . "</td><td>" . $row["1"] . "</td><td>" . $row["2"] . "</td><td>" . $row["3"] . "</td></tr>"; //or just use "echo $row[0]"
                }
            } else {
        ...
        function displayDailyReport() {
            ...
            printResult($result);
            // number of vehicles per category return today for specific branch
            $numOfVehiclesPerCat = executePlainSQL(
                "SELECT COUNT(*), city, location, vehicles.vtName FROM rentals JOIN vehicles ON rentals.vlicense = vehicles.vlicense JOIN returns ON returns.rid = rentals.rid WHERE retime >= TO_DATE('$startDateTime','yyyy-mm-dd hh24:mi:ss') AND retime <= TO_DATE('$endDateTime', 'yyyy-mm-dd hh24:mi:ss') AND vehicles.city = '$city' AND vehicles.location = '$location' GROUP BY city, location, vehicles.vtName ORDER BY vehicles.vtName, city, location");
            ...
            // total revenue for specific branch
            $grandPerBranch = executePlainSQL(
                "SELECT SUM(value), vehicles.city, vehicles.location FROM rentals JOIN vehicles ON rentals.vlicense = vehicles.vlicense JOIN returns ON returns.rid = rentals.rid WHERE retime >= TO_DATE('$startDateTime','yyyy-mm-dd hh24:mi:ss') AND retime <= TO_DATE('$endDateTime', 'yyyy-mm-dd hh24:mi:ss') AND vehicles.city = '$city' AND vehicles.location = '$location' GROUP BY city, location ORDER BY city, location");
            printTotalRevenueForBranch($grandPerBranch);
            // total revenue per category for specific branch
            $grandPerCat = executePlainSQL("SELECT SUM(returns.value), city, location, vehicles.vtName FROM rentals JOIN vehicles ON rentals.vlicense = vehicles.vlicense JOIN returns ON returns.rid = rentals.rid WHERE retime >= TO_DATE('$startDateTime','yyyy-mm-dd hh24:mi:ss') AND retime <= TO_DATE('$endDateTime', 'yyyy-mm-dd hh24:mi:ss') AND vehicles.city = '$city' AND vehicles.location = '$location' GROUP BY city, location, vehicles.vtName ORDER BY vehicles.vtName, city, location");
            printTotalRevenueForCat($grandPerCat);
        }
